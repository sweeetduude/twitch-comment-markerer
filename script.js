...
function activateOnPage() {
  ...
  $buttonHighlight.innerHTML = "Highlight message";
  $buttonHighlight.addEventListener("click", () => {
    if (!ENDPOINT_URL) {
      alert("Set endpoint url in addon settings!");
      return;
    }
    const $preExistingArea = document.querySelector(
      `.${COMMENT_AREA_CLASSNAME}`
    );
  ...
  $buttonHighlight.style.position = "absolute";
  $buttonHighlight.style.left = "2px;";
  $buttonHighlight.style.top = "2px";
  $buttonHighlight.style.zIndex = 99;
  ...
  $container.addEventListener("dblclick", (e) => {
    ...
    if ($chatLine) {
      ...
      if (!markerer) {
        let $preExistingMarkerers = document.querySelector(
          `.${INLINE_AREA_CLASSNAME}`
        );
        ...
        $buttonMark.addEventListener("click", () => {
          ...
          const chatData = {
            ...
            username: $chatLine.querySelector(".chat-author__display-name")
              .innerText,
            message: Array.from(
              (
                $chatLine.querySelector(".message") ||
                $chatLine.querySelector(
